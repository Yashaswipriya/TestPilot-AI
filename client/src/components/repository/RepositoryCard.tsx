...
export function RepositoryCard({ repository }: RepositoryCardProps) {
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      await repositoryService.importRepository(
        repository.owner,
        repository.name
      );
      ...
      >
        ...
        <div className="mt-auto flex items-center justify-between pt-4 border-t border-border/50">
          <div className="flex items-center gap-4 font-mono text-[11px] text-faint uppercase font-medium tracking-wider">
            {repository.language && (
              <div className="flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-foreground opacity-80" />
                {repository.language}
              </div>
            )}
          ...
          <Button
            ...
            className="opacity-0 transition-opacity group-hover:opacity-100 font-mono tracking-tight text-[12px] h-7 px-3"
          >
            {isImporting ? 'Opening...' : 'Open'}
          </Button>
        </div>
      </motion.div>
    </Link>
